...
example-stt: stt-$(shell go env GOOS)-$(shell go env GOARCH)
	...
	( if [ "$(AUDIO_SOURCE_PATH)" = '' ]; then arecord -f FLOAT_LE -c 1 -r 16000; else cat "$(AUDIO_SOURCE_PATH)"; fi ) | ./build/stt-$(shell go env GOOS)-$(shell go env GOARCH) --log-level trace --language en-US --translate=true --alignment-aheads-preset $(subst -,_,$(WHISPER_MODEL)) thirdparty/whisper.cpp/models/ggml-"$(WHISPER_MODEL)".bin | jq -r 'select(.IsFinal) | .Variants.[] | .Text'
